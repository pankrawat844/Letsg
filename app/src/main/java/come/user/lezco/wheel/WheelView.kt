...
class WheelView : View {
    ...
    // Scrolling listener
    internal var scrollingListener: WheelScroller.ScrollingListener =
    ...
    fun setCurrentItem(index: Int, animated: Boolean = false) {
        var index = index
        if (this.viewAdapter == null || this.viewAdapter!!.itemsCount == 0) {
            return  // throw?
        }

        val itemCount = this.viewAdapter!!.itemsCount
    ...
    private fun calculateLayoutWidth(widthSize: Int, mode: Int): Int {
        ...
        itemsLayout!!.measure(
            View.MeasureSpec.makeMeasureSpec(width - 2 * PADDING, View.MeasureSpec.EXACTLY),
            View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED)
        )
    ...
    private fun isValidItemIndex(index: Int): Boolean {
        return this.viewAdapter != null && this.viewAdapter!!.itemsCount > 0 &&
                (isCyclic || index >= 0 && index < this.viewAdapter!!.itemsCount)
    }
    ...
    private fun getItemView(index: Int): View? {
        ...
        val count = this.viewAdapter!!.itemsCount
